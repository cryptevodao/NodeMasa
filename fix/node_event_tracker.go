...
func (net *NodeEventTracker) HandleNodeData(data NodeData) {
    ...
    if !ok {
        ...
        newData := data // Create a new NodeData object
        net.nodeData[data.PeerId.String()] = &newData
        return
    }
...
func (net *NodeEventTracker) GetAllNodeData() []NodeData {
    ...
    net.dataMutex.RLock() // Acquire a read lock
    defer net.dataMutex.RUnlock() // Ensure the lock is released at the end

    // Create a temporary slice to hold the data
    var nodeDataSlice []NodeData

    // Iterate over the map and copy data to the slice
